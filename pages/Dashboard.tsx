...
import { ArrowUpRight, ArrowDownRight, Download, Save, FileText, Target, Wallet, Activity, TrendingUp, TrendingDown, ArrowUpCircle, ArrowDownCircle, PieChart as PieChartIcon, Sun, Moon, Calendar, Clock } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ transactions, budgets, userName }) => {
  ...
  const savingsRate = useMemo(() => {
     if (summary.income === 0) return 0;
     return Math.max(0, ((summary.income - summary.expense) / summary.income) * 100);
  }, [summary]);
  ...
  const handleExportPDF = () => {
    ...
    doc.save(`smart_expense_report_${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `smart_expense_export_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const { chartData, incomeTrend, expenseTrend } = useMemo(() => {
    ...
    const curInc = currentMonthTxs.filter(t => t.type === 'income').reduce((a, b) => a + b.amount, 0);
    const prevInc = prevMonthTxs.filter(t => t.type === 'income').reduce((a, b) => a + b.amount, 0);
    
    const curExp = currentMonthTxs.filter(t => t.type === 'expense').reduce((a, b) => a + b.amount, 0);
    const prevExp = prevMonthTxs.filter(t => t.type === 'expense').reduce((a, b) => a + b.amount, 0);
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dayStr = date.toLocaleString('default', { day: 'numeric', month: 'short' });
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl z-50">
          <p className="text-xs font-bold text-slate-700 dark:text-slate-200 mb-2">{label}</p>
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex items-center gap-3 text-xs font-medium mb-1 last:mb-0">
              <div className="flex items-center gap-1.5 min-w-[60px]">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.color }} />
                <span className="text-slate-500 dark:text-slate-400 capitalize">{entry.name}</span>
              </div>
              <span className="text-slate-900 dark:text-white font-mono ml-auto">{formatCurrency(entry.value)}</span>
            </div>
          ))}
        </div>
      );
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <motion.div 
          ...
          className="lg:col-span-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 relative overflow-hidden shadow-sm hover:shadow-md transition-shadow"
        >
          ...
          <div className="flex justify-between items-start mb-8 relative z-10">
            <div className="flex gap-4">
              ...
              <div>
                <p className="text-slate-500 dark:text-slate-400 font-medium text-sm mb-1">Monthly Budget</p>
            ...
            <div className="text-right">
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-1">Spent</p>
          ...
          <div className="relative pt-2 z-10">
            <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-2xl h-3 mb-2 overflow-hidden">
               <motion.div 
                 initial={{ width: 0 }}
                 animate={{ width: `${Math.min((budgetSummary.totalSpent / (budgetSummary.totalLimit || 1)) * 100, 100)}%` }}
            ...
            <div className="flex justify-end">
              <span className="text-xs font-medium text-slate-500 dark:text-slate-500">
                {Math.max(0, (100 - (budgetSummary.totalSpent / (budgetSummary.totalLimit || 1)) * 100)).toFixed(1)}% remaining
              </span>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="text-center -mt-2">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Savings Rate: <span className="text-slate-900 dark:text-white">{savingsRate.toFixed(1)}%</span></p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
           ...
           <div className="flex items-center gap-2 mt-auto">
              ...
              <span className={`text-sm font-medium ${incomeTrend >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
        ...
        >
           ...
           <div className="flex items-center gap-2 mt-auto">
              ...
              <span className={`text-sm font-medium ${expenseTrend <= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div 
          variants={itemVariants}
          className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-sm"
        >
          <div className="flex justify-between items-start mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm">Last 30 days performance</p>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-emerald-500"></span>
                <span className="text-sm text-slate-500 dark:text-slate-400">Income</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-rose-500"></span>
                <span className="text-sm text-slate-500 dark:text-slate-400">Expense</span>
              </div>
            </div>
          </div>
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <BarChart data={chartData} barGap={4} barCategoryGap="20%">
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 10, fontWeight: 500}} dy={10} interval={2} />
                <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 10, fontWeight: 500}} tickFormatter={(value) => `${value >= 1000000 ? (value/1000000).toFixed(1) + 'M' : (value/1000).toFixed(0) + 'k'}`} dx={-10} />
                <Tooltip content={<CustomTooltip />} cursor={{ fill: '#f1f5f9', opacity: 0.1 }} />
                <Bar dataKey="income" fill="#10b981" radius={[2, 2, 0, 0]} maxBarSize={15} animationDuration={1500} />
                <Bar dataKey="expense" fill="#f43f5e" radius={[2, 2, 0, 0]} maxBarSize={15} animationDuration={1500} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-xs text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 font-medium transition-colors">View All</button>
          </div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto max-h-[350px] pr-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
            {recentTransactions.length === 0 ? (
               <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-2">
                 <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-full"><Activity size={20}/></div>
              ...
              recentTransactions.map((t, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-900 dark:text-white truncate max-w-[120px]">{t.title}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
           <div className="flex items-center gap-3 mb-6">
             ...
             <h3 className="text-lg font-bold text-slate-900 dark:text-white">Income Breakdown</h3>
           </div>
           ...
           <div className="h-64 flex flex-col md:flex-row items-center gap-4">
              ...
              <div className="w-full md:w-1/2 space-y-3">
                 {incomeBreakdown.length === 0 ? (
                    <p className="text-slate-500 text-sm text-center">No income data available</p>
                 ) : (
           ...
           <div className="h-64 flex flex-col md:flex-row items-center gap-4">
              ...
              <div className="w-full md:w-1/2 space-y-3">
                 {expenseBreakdown.length === 0 ? (
                    <p className="text-slate-500 text-sm text-center">No expense data available</p>
                 ) : (
